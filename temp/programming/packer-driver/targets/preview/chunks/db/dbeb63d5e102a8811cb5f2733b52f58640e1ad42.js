System.register(["__unresolved_0", "__unresolved_1"], function (_export, _context) {
  ...
  var ObjectUtil, StringUtil, Broadcaster;
  return {
    setters: [function (_unresolved_) {
      ObjectUtil = _unresolved_.ObjectUtil;
    }, function (_unresolved_2) {
      StringUtil = _unresolved_2.StringUtil;
    }],
    execute: function () {
      _export("Broadcaster", Broadcaster = class Broadcaster {
        constructor() {
        ...
        _checkListenerValidity(key) {
          if (!key) return;
          var handlers = this._listenerHandlerMap[key];
        ...
        _addHandler(handler) {
          var handlerMap = this._listenerHandlerMap;
          var msgKey = handler.key;
          var handlers = handlerMap[msgKey] || [];
          handlers.push(handler);
          handlerMap[msgKey] = handlers; //检查是否有粘性广播

          var stickyHandlers = this._stickBrodcastMap[msgKey];
        ...
        _onHander(keyOrHandler, listener, context, once, args) {
          if (typeof keyOrHandler === "string") {
            if (!listener) return;
            var handlerObj = this._unuseHandlers.pop() || {};
        ...
        off(key, listener) {
          ...
          var index = handlers.findIndex(handler => handler.listener === listener);
          var handler = handlers.fastRemoveAt(index);
        ...
        offAll(key, context) {
          var handlerMap = this._listenerHandlerMap; //指定key或全局清除

          var processHandler = (handlers, msgKey, hasContext) => {
            for (var i = handlers.length - 1; i >= 0; i--) {
              var shouldRemove = !hasContext || handlers[i].context === context;
        ...
        */
        ...
        dispose() {
          //@ts-ignore
          this._listenerHandlerMap = undefined; //@ts-ignore

          this._stickBrodcastMap = undefined; //@ts-ignore
...
//# sourceMappingURL=dbeb63d5e102a8811cb5f2733b52f58640e1ad42.js.map