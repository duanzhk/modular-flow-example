System.register(["__unresolved_0", "reflect-metadata"], function (_export, _context) {
  ...
  function getInterface(ctor) {
    var sym = interfaceSymbols.get(ctor);
  ...
  function CleanInjectedProperties(constructor) {
    return class extends constructor {
      constructor() {
        super(...arguments); // 递归收集当前类及其所有父类的注入属性

        var collectInjectedProperties = klass => {
          if (klass === null || klass === Object) return [];
          var parentProperties = collectInjectedProperties(Object.getPrototypeOf(klass));
          var currentProperties = Reflect.getMetadata(INJECTED_PROPERTIES_KEY, klass) || []; // const currentProperties :any[] = []

          return [...parentProperties, ...currentProperties];
        }; // 合并并去重属性名


        var injectedProperties = [...new Set(collectInjectedProperties(constructor))]; // 删除实例上的所有注入属性
  ...
  function managedWithClean() {
    return function (ctor) {
      // 先执行清理逻辑
      var decoratedCtor = CleanInjectedProperties(ctor); // 后执行注册逻辑
  ...
  function injectManager(sym) {
    return function (target, prop) {
      var injectionKey = Symbol.for(prop);
      ...
      var injectedProperties = Reflect.getMetadata(INJECTED_PROPERTIES_KEY, target.constructor) || [];
